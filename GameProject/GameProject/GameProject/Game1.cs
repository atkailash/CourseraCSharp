...
namespace GameProject
{
    ...
    public class Game1 : Microsoft.Xna.Framework.Game
    {
        ...
        private void SpawnBear()
        {
            ...
            int xLoc = GetRandomLocation(GameConstants.SPAWN_BORDER_SIZE, GameConstants.WINDOW_WIDTH);
            int yLoc = GetRandomLocation(GameConstants.SPAWN_BORDER_SIZE, GameConstants.WINDOW_HEIGHT);
            // generate random velocity
            float bearSpeed = GameConstants.MIN_BEAR_SPEED + RandomNumberGenerator.NextFloat(GameConstants.BEAR_SPEED_RANGE);
            float bearAngle = RandomNumberGenerator.NextFloat(2*(float)Math.PI);
            Vector2 bearVec = new Vector2((bearSpeed*(float)Math.Cos((double)bearAngle)), (bearSpeed*(float)Math.Sin((double)bearAngle)));
            // create new bear
            TeddyBear newBear = new TeddyBear(Content, "teddybear", xLoc, yLoc, bearVec, null, null);
